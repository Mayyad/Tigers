...
{  
       ...
       function __construct() {  
		...
		/************************* View My Last Order On DataBase *************************/
		function viewMyLastOrder($u_id)
		{
			$db = dbConnect::getInstance();
    		 $mysqli = $db->getConnection();
			$query = " select * from check_tb  where u_id = '".$u_id."' ORDER BY ID DESC limit 1 ";  
            $res = $mysqli->query($query) or die (mysqli_error($mysqli));
			if(mysqli_num_rows($res) > 0)
			{
				$row=mysqli_fetch_array($res);
				$query2="select * from orders_tb where check_id='".$row['id']."'";
				$orderRes=$mysqli->query($query2)or die(mysqli_error($mysqli));
				while($rowOrder = mysqli_fetch_array($orderRes))
				{
					$query3="select * from products_tb where id='".$rowOrder['prod_id']."'";
					$productRes=$mysqli->query($query3)or die(mysqli_error($mysqli));
